...
while true; do
	if [ -e /system/etc/init.d/SS99RTMixManager ] && [ -e /system/etc/usrsettings ] && [ -e /system/etc/readme.html ]; then
		...
		if [ "$rtmixman_currentver" -gt "$rtmixman_checkver" ]; then
		...
		elif [ "$rtmixman_currentver" -eq "$rtmixman_checkver" ]; then
			setprop rtmixman_inst null
			echo "you have the latest version $rtmixman_currentver of         "
			echo "                       ...this engine installed."
		elif [ "$rtmixman_currentver" -lt "$rtmixman_checkver" ]; then
			setprop rtmixman_inst wise
		...
		elif [ "$rtmixman_currentver" == null ]; then
			setprop rtmixman_inst wise
...
(while true; do
	...
	REALTOTALMEM=\$((\$TOTALMEM-\$MFK))
	lowpage=\$((\$MFK/4))
	requiredsize=\$((\$REALTOTALMEM/6))
	requiredpage=\$((\$requiredsize/4))
	requiredsafety=\$((\$requiredpage/256))
	pagelimiter=\$requiredsafety
	if [ -e "/proc/\`pgrep launcher\`/status" ]; then
		launchersize=\`cat /proc/\$(pgrep launcher)/status | grep VmRSS | awk '{print \$2}'\`
		launcherpage=\$((\$launchersize/4))
		launchersafety=\$((\$launcherpage/256))
		if [ "\$launchersize" -le "\$requiredsize" ]; then
			if [ "\`cat /proc/\$(pgrep launcher)/oom_adj\`" != -16 ]; then
				echo "-16" > /proc/\`pgrep launcher\`/oom_adj
			fi
			pagelimiter=\$((\$pagelimiter+\$launchersafety))
		fi
	fi
	if [ -e "/proc/\`pgrep acore\`/status" ]; then
		acoresize=\`cat /proc/\$(pgrep acore)/status | grep VmRSS | awk '{print \$2}'\`
		acorepage=\$((\$acoresize/4))
		acoresafety=\$((\$acorepage/256))
		if [ "\$acoresize" -le "\$requiredsize" ]; then	
			if [ "\`cat /proc/\$(pgrep acore)/oom_adj\`" != -12 ]; then
				echo "-12" > /proc/\`pgrep acore\`/oom_adj
			fi
			pagelimiter=\$((\$pagelimiter+\$acoresafety))
		fi
	fi
	pagelimiter=\$((pagelimiter*256))
	if [ -e "/sys/module/lowmemorykiller/parameters/minfree" ]; then
		if [ "\$REALTOTALMEM" -le 524288 ]; then
			if [ \`cat /sys/module/lowmemorykiller/parameters/minfree\` != "\$lowpage,\$pagelimiter,\$pagelimiter,\$lowpage,\$lowpage,\$pagelimiter" ]; then
				echo "\$lowpage,\$pagelimiter,\$pagelimiter,\$lowpage,\$lowpage,\$pagelimiter" > /sys/module/lowmemorykiller/parameters/minfree
			fi
		elif [ "\$REALTOTALMEM" -le 1048576 ]; then
			if [ \`cat /sys/module/lowmemorykiller/parameters/minfree\` != "\$lowpage,\$lowpage,\$pagelimiter,\$lowpage,\$lowpage,\$pagelimiter" ]; then
				echo "\$lowpage,\$lowpage,\$pagelimiter,\$lowpage,\$lowpage,\$pagelimiter" > /sys/module/lowmemorykiller/parameters/minfree
			fi
		elif [ "\$REALTOTALMEM" -le 2097152 ]; then
			if [ \`cat /sys/module/lowmemorykiller/parameters/minfree\` != "\$lowpage,\$lowpage,\$pagelimiter,\$lowpage,\$lowpage,\$lowpage" ]; then
				echo "\$lowpage,\$lowpage,\$pagelimiter,\$lowpage,\$lowpage,\$lowpage" > /sys/module/lowmemorykiller/parameters/minfree
			fi
		else
			if [ \`cat /sys/module/lowmemorykiller/parameters/minfree\` != "\$lowpage,\$lowpage,\$pagelimiter,\$lowpage,\$lowpage,\$lowpage" ]; then
				echo "\$lowpage,\$lowpage,\$pagelimiter,\$lowpage,\$lowpage,\$lowpage" > /sys/module/lowmemorykiller/parameters/minfree
