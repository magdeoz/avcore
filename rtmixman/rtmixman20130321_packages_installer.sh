...
while true;
	do
		...
		echo "choose (c)ustom mode to determine what to install all by yourself.";
...
while true;
	do
		...
		case $i in
			...
			sleep 2;
			mpe=0
			echo "hmm...okay:(, just wanted to let you know that you just disabled the most SPECIAL part of this script...";
			break;
			;;
			*)
			echo "";
...
echo "";
echo "all setup options have been selected,"
sleep 2;
echo "and now its ready to be installed onto your device!";
sleep 2;
echo "";
...
eps=\$((\$dpsg+1));
fpsg=\$((\$eps+1));
#try to block service apps from being terminated.
if [ \$adj1 == 1 ];
	then
...
if [ -s /system/etc/init.d/SS99RTMixManager -o -s /system/etc/usrsettings -o -s /system/etc/readme.html -o -s /system/xbin/rtmixman -o -s /system/etc/init.d/SS98enable_touchscreen -o -s /system/etc/init.d/SS97Netweaks ];
	then
		#version check.
		setprop rtmixman_checkver 20130321
...
while true;
	do
		...
		echo "";
		echo "          1)gamer mode";
		echo "          didntisaythisplaceisunderconstruction...";
		echo "          Y U HAD TO PRESS IT???";
		echo "          ";
		echo "          ";
		echo "          ";
		echo "          ";
