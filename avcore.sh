...
readonly avcore_ver="0.0.6 alpha"
...
$BASE_NAME \e[1;33mv$avcore_ver\e[0m
		...
		if [[ "$opt_p_val" ]]; then
			if [[ "$opt_p_val" -gt 200 ]] || [[ "$opt_p_val" -lt 5 ]]; then
				if [[ "$opt_p_val" -gt 9000 ]]; then
					debug(){
						echo "welcome to the avcore debugging program!"
						echo  -e -n "\e[1;32mdebug-\e[1;33m$avcore_ver\e[0m\$ "
						while eval read i; do
							case $i in
								...
								;;
								help)
									echo "this debug shell is ONLY for testing conditions inside the program!
it is not a complete shell so you CANNOT use some special syntax with it.
such includes:
	-functions
	-dollar\(\$\) signs
	-and stuff..
