...
main(){
	while true; do
		...
		if [[ ! -f $external/wakelocksheriff_pid ]]; then
			echo null > $external/wakelocksheriff_pid
		fi
		if [[ "$?" != 0 ]]; then
			error something went wrong.
			exit 1
		...
		if [[ "$appliedonboot" ]]&&[[ ! -s $external/rtmixman_pid ]]&&[[ ! -s $external/singlecorefix_pid ]]&&[[ ! -s $external/wakelocksheriff_pid ]]; then
		...
		if [[ "$(cat $external/wakelocksheriff_pid)" != null ]]&&[[ "$(ps | grep "$(cat $external/wakelocksheriff_pid)" | grep -v grep)" ]]; then
...
q)exit'
		...
		case $f in
			...
			5)
				if [[ "$(cat $external/wakelocksheriff_pid)" != null ]]&&[[ "$(ps | grep "$(cat $external/wakelocksheriff_pid)" | grep -v grep)" ]]; then
					kill -9 $(cat $external/wakelocksheriff_pid)
				else
					echo -n 'how many seconds interval?: >>'
					read time
					echo 'Garbageprocess Disposer - in how much cpu usage should the Disposer limit?(out of 100% cpu usage):'
